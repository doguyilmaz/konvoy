...
import { commandTable, FLAG_VALUES, GLOBAL_FLAGS } from './table'
...
function bash(): string {
  ...
  const flagCases = commandTable
    .filter((c) => Object.keys(c.flags).length > 0)
    .map((c) => `    ${[c.name, ...c.aliases].join('|')}) flags="${words(longFlags(Object.keys(c.flags)))} $flags" ;;`)
    .join('\n')
...
${flagCases}
  ...
  if [ "$COMP_CWORD" -eq 2 ]; then
    case "$cmd" in
      ...
      help) COMPREPLY=($(compgen -W "${words(names)}" -- "$cur")) ;;
    esac
  fi
}
...
function zsh(): string {
  ...
  const flagCases = commandTable
    .filter((c) => Object.keys(c.flags).length > 0)
    .map((c) => `    ${[c.name, ...c.aliases].join('|')}) flags+=(${longFlags(Object.keys(c.flags)).join(' ')}) ;;`)
    .join('\n')
...
${flagCases}
  ...
  if (( CURRENT == 3 )); then
    case $words[2] in
      ...
      help) compadd -- ${words(commandTable.flatMap((c) => [c.name, ...c.aliases]))} ;;
    esac
  fi
}
...
function fish(): string {
  ...
  for (const c of commandTable) {
    ...
    for (const [f, help] of Object.entries(c.flags)) {
      const value = FLAG_VALUES[f] ? ' -x' : ''
      const agents = f === 'lead' ? ` -a ${fq(words(agentIds))}` : f === 'via' ? ` -a 'script npm brew bun'` : ''
      lines.push(`complete -c konvoy -n '__fish_seen_subcommand_from ${[c.name, ...c.aliases].join(' ')}' -l ${f}${value}${agents} -d ${fq(help)}`)
    }
  }
  ...
  lines.push(`complete -c konvoy -n '__fish_seen_subcommand_from help' -a ${fq(words(commandTable.flatMap((c) => [c.name, ...c.aliases])))}`)
  return `${lines.join('\n')}\n`
}
