...
export const mutations: Mutation[] = [
  ...
  // --- the audit of 2026-09-24: the prelude window, streaming, the editor, privacy ---
  {
  ...
  {
    name: 'the goal is no longer given on first contact, so the first agent of a session never hears it',
    file: 'src/core/prelude.ts',
    from: "if (total === 0) return window.firstContact && session.goal !== '' ? `goal: ${session.goal}` : ''",
    to: "if (total === 0) return ''",
    tests: ['tests/prelude.test.ts', 'tests/session.test.ts'],
  },
  {
    name: "a failover successor's window runs past the send's start, quoting the blocked attempt at the same question",
    file: 'src/core/session.ts',
    from: 'const prelude = await preludeFor(current, { upTo }, facts)',
    to: 'const prelude = await preludeFor(current, {}, facts)',
    tests: ['tests/session.test.ts'],
  },
  {
    name: 'git facts are gathered on every turn, including the ones whose prelude quotes nothing',
    file: 'src/core/session.ts',
    from: "hasUnseenTurns(deps.db, session, opts) ? await facts() : ''",
    to: 'await facts()',
    tests: ['tests/session.test.ts'],
  },
  {
    name: "claude's streamed text is read again from the message that repeats it, doubling every answer",
    file: 'src/adapters/claude.ts',
    from: 'text: id !== null && streamedText.has(id),',
    to: 'text: false,',
    tests: ['tests/adapter-claude.test.ts'],
  },
  {
    name: 'claude is no longer asked for partial messages, so its answer arrives a block at a time',
    file: 'src/adapters/claude.ts',
    from: "'--verbose', '--include-partial-messages']",
    to: "'--verbose']",
    tests: ['tests/adapter-claude.test.ts'],
  },
  {
    name: 'streamed text reaches the terminal unsanitised, so an echoed escape sequence runs',
    file: 'src/render.ts',
    from: '    const chunk = safeText(raw)\n',
    to: '    const chunk = raw\n',
    tests: ['tests/render.test.ts'],
  },
  {
    name: 'an interrupted turn keeps whatever error the dying agent left instead of reading as interrupted',
    file: 'src/core/turn.ts',
    from: "      result.error = { message: 'interrupted', kind: 'interrupted' }\n",
    to: '',
    tests: ['tests/turn.test.ts'],
  },
  {
    name: 'the abort signal is never listened to, so Esc cannot stop a turn',
    file: 'src/core/turn.ts',
    from: "  opts.signal?.addEventListener('abort', interrupt, { once: true })\n",
    to: '',
    tests: ['tests/turn.test.ts'],
  },
  {
    name: 'every streamed delta is stored as its own event row, a write per token',
    file: 'src/core/turn.ts',
    from: "    if (event.t === 'text') heldText += event.text\n    else {",
    to: '    {',
    tests: ['tests/turn.test.ts'],
  },
  {
    name: 'a flag a command does not read is ignored again, so a mistyped --session runs against the current session',
    file: 'src/commands/table.ts',
    from: '  if (stray === undefined) return null',
    to: '  return null',
    tests: ['tests/cli.test.ts'],
  },
  {
    name: 'a first Ctrl-C on an empty line leaves at once instead of arming the exit',
    file: 'src/editor.ts',
    from: "        if (armed) return { t: 'exit' }\n        this.exitArmed = true",
    to: "        return { t: 'exit' }",
    tests: ['tests/editor.test.ts'],
  },
  {
    name: 'a large paste is sent as its placeholder instead of what was pasted',
    file: 'src/editor.ts',
    from: 'return this.buffer.replace(PLACEHOLDER, (whole, n: string) => this.pastes.get(Number(n)) ?? whole)',
    to: 'return this.buffer',
    tests: ['tests/editor.test.ts'],
  },
  {
    name: 'Esc during a turn is kept as type-ahead instead of stopping the turn',
    file: 'src/editor.ts',
    from: "if (chunk === '\\x1b' || chunk.includes('\\x03')) {",
    to: "if (chunk.includes('\\x03')) {",
    tests: ['tests/editor.test.ts'],
  },
  {
    name: 'a paste split mid end-marker loses the marker, so the paste never ends',
    file: 'src/editor.ts',
    from: '            while (keep > 0 && !PASTE_END.startsWith(pending.slice(pending.length - keep))) keep--\n',
    to: '            keep = 0\n',
    tests: ['tests/editor.test.ts'],
  },
  {
  ...
  },
  {
    ...
    to: '  const found = true',
    tests: ['tests/send.test.ts'],
  },
  {
    name: 'a codex item that failed through its status reports as a success',
    file: 'src/adapters/codex.ts',
