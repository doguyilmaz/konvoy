...
**Brief** - `.konvoy/<slug>/CONTEXT.md`, written at creation with the goal and appended by
`/goal`. No agent is pointed at it; the goal reaches agents through the prelude.
...
agent. Roles are hints for delegation, not enforcement. The lead is the agent a new session
starts with, the one bare `konvoy` talks to first; with none configured it is claude.
...
```
src/
  cli.ts          flags, dispatch, interactive mode entry
  ...
  core/
    ...
    prelude.ts    what the next agent is told (§28) and envelope parsing (§21)
    facts.ts      machine facts (§29)
    gate.ts       the gate (§32)
...
There is no MCP server. Delegation is the envelope of §21: with `delegation.enabled`, every turn
is told how to end with a `<<<konvoy` block naming `to:` and `task:`. konvoy resolves `to` as a
role or an agent name, runs that agent once with the task as its prompt and a prelude carrying
the sender's envelope (§28), and prints the handoff. A recipient that is disabled or not
installed is reported instead of silently dropped.

One hop per `send`: an envelope in the recipient's answer is returned, not followed, so a loop
cannot form and no depth or cycle guard is needed. Each CLI's own subagents keep working
...
Serialized: one turn per session at a time, enforced by the session lock (§20). Simple, and
correct for the common case.

Not built: `--parallel`, one git worktree per agent with konvoy merging at the end and
reporting conflicts rather than resolving them, and `--tmux`, turns inside tmux sessions named
`konvoy_<slug>_<agent>` so a running agent can be watched mid-turn. Both are borrowed from amux
and sit on the roadmap (§33).
...
| delegation loop | cannot form: one hop per `send` (§10) |
| concurrent writes | serialized by the session lock (§20) |
...
try to steer the whole convoy, and a handoff is the widest path for it: the recipient runs the
sender's `task` as its prompt.

- **konvoy never raises privilege.** A handed-off turn runs at the recipient's own configured
  permission; an envelope cannot ask for more, and `permission` is privileged (global config
  only, §12).
- **The author is always named.** The prelude opens every handoff with `<sender> handed off
...
`KONVOY_LEASE`. An agent that runs `konvoy send` itself during its turn would otherwise ask for
a lock its own parent already holds and deadlock.

- The nested konvoy inherits `KONVOY_LEASE` from the agent process and presents it. A lease
  matching the current holder is admitted without re-acquiring.
...
Designed, and only partly built (`to`, `task`, `open` and `decisions` are parsed; the machine facts
arrive as the table of §29), konvoy normalises that, plus its own git facts, into one record:
